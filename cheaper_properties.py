# Class defined earlier, added def cheaper properties which needed to be two lines with list comprehension.
class RealProperty:
...
# This is the code written for the task:
def cheaper_properties(properties: list, reference: RealProperty):
    return [(p, (reference.price_per_sqm * reference.square_meters) - (p.price_per_sqm * p.square_meters)) for p in properties if (p.price_per_sqm * p.square_meters) < (reference.price_per_sqm * reference.square_meters)]
